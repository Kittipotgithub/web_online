...
@Component({
  selector: 'app-ns032',
  templateUrl: './ns032.component.html',
  styleUrls: ['./ns032.component.scss'],
  providers: [{ provide: DateAdapter, useClass: AppDateAdapter }, { provide: MAT_DATE_FORMATS, useValue: APP_DATE_FORMATS }]

})
export class Ns032Component implements OnInit {
  ns032FormSearch: FormGroup;
  ...
  createFormGroup() {
    this.ns032FormSearch = this.formBuilder.group({
  ...
  onSearch(type) {
    const form = this.ns032FormSearch.value;
  ...
  selectDocument(document) {
    this.router.navigate(['ns03/ns031'], {
