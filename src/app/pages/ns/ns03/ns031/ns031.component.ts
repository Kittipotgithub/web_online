...
export class Ns031Component implements OnInit {
  ...
  ownCenterCodeControl: FormControl; // 	รหัสศูนย์ต้นทุนเจ้าของรายได้
  ...
  createFormControl() {
    ...
    this.ownCenterCodeControl = this.formBuilder.control('') //	รหัสศูนย์ต้นทุนเจ้าของรายได้
  ...
  createFormGroup() {
    this.ns031FormCreate = this.formBuilder.group({
      ...
      ownCenterCode: this.ownCenterCodeControl, //	รหัสศูนย์ต้นทุนเจ้าของรายได้
  ...
  defaultInput() {
    ...
    this.ns031FormCreate.patchValue({
      ...
      typeReceive: this.constant.LIST_TYPE_RECEIVE_MONEY_NS03[0].id, // ประเภทการรับเงิน
      reference: '', // การอ้างอิง
      typeMoney: this.constant.LIST_TYPE_MONEY_NS03[0].id, // ประเภทเงิน
      bankBook: '',// Bank book
      ...
      ownCenterCode: '',//	รหัสศูนย์ต้นทุนเจ้าของรายได้
  ...
  setInputFormSelect(item) {
    ...
    this.ownCenterCodeBindingName=item.ownCenterName;
  ...
  setInputFromSearchToForm(head, items) {
    ...
    //งงงงอยู่
    this.ownCenterCodeBindingName = item40.costCenterName;
  ...
  saveIntoList(value) {
    ...
    value.ownCenterName=this.ownCenterCodeBindingName;
  ...
  searchDocumentDetail(docNo, docYear) {
    ...
    const payload: SearchFiDetailBody = {
      ...
      formId: this.constant.FORM_ID_NS.ns03,
  ...
  openDialogPreSave() {
    const allPageNs03 = new Map();
    allPageNs03.set('path', 'ns03');
    allPageNs03.set('create', 'ns031');
    allPageNs03.set('search', 'ns032');
    allPageNs03.set('backList', 'ns033');
    this.listValidate = [];
    ...
    if (this.listValidate.length <= 0) {
      if (this.listDocument.length > 0) {
        const form = this.ns031FormCreate.value;
        console.log(form);
        ...
        header = {
          ...
          costCenter1: '',
          costCenter2: form.ownCenterCode,
          currency: 'THB',
  ...
  //       this.ns031FormCreate.patchValue({ [result.type]: result.value });
  //     }
  //   });
  // }
  openDialogSearch(type) {
    ...
    if (type === 'vendorBankAccount') {
      specialCase = {
        vendorTaxId: this.ns031FormCreate.get('vendorTaxId').value
      };
    }
      ...
      if (result && result.event) {
        ...
        if (type === 'vendorTaxId') {
          ...
          if (result.value != undefined || result.value != null || result.value != '') {
            this.ns031FormCreate.controls.vendorBankAccount.enable();
          }
        }
        if (type === 'typeAccountCode') {
        ...
        }else if (type === 'ownCenterCode') {
          this.ownCenterCodeBindingName = result.name;
          ...
          this.ns031FormCreate.patchValue({ ownAccountDepositCode: result.optional });
  ...
  onBlurSearch(type) {
    ...
    if (type === 'typeAccountCode') {
      ...
      this.masterService.findOneDataCenter(this.userProfile.userdata.departmentCode, form.centerCode).subscribe(data => {
        ...
        if (response) {
          this.ns031FormCreate.patchValue({
            ownCenterCode: response.valueCode
          });
          this.ownCenterCodeBindingName = response.name;
        }
      });
    } else if (type === 'sourceMoneyCode') {
